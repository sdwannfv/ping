...
const (
	...
	FlagTimeStamp                          // pass the linklayer timestamp on the received packet
)
...
func (cm *ControlMessage) Parse(b []byte) error {
	...
	for _, m := range ms {
		...
		if lvl == unix.SOL_SOCKET && typ == ctlOpts[ctlTimeStamp].name && l >= ctlOpts[ctlTimeStamp].length {
			ctlOpts[ctlTimeStamp].parse(cm, m.Data(l))
		} else if lvl != iana.ProtocolIP {
			continue
		}
		switch {
...
func NewControlMessage(cf ControlFlags) []byte {
	...
	var l int
	if opt.isset(FlagTimeStamp) && ctlOpts[ctlTimeStamp].name > 0 {
		l += socket.ControlMessageSpace(ctlOpts[ctlTimeStamp].length)
	}
...
const (
	...
	ctlTimeStamp         // inbound timestamp
	ctlMax
)
